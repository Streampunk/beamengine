...
module.exports = {
  ...
  redis: {
    ...
    cluster: process.env.BEAM_DEV_REDIS_CLUSTER === 'true',
    // IP address of primary redis host
    host: process.env.BEAM_DEV_REDIS_HOST || '127.0.0.1',
    // Port number of primary redis host connection
    port: +process.env.BEAM_DEV_REDIS_PORT || 6379,
    // Password to authenticate with redis (optional)
    password: process.env.BEAM_DEV_REDIS_PASSWORD || null,
    // Redis database to be used for main cache
    db: +process.env.BEAM_DEV_REDIS_DB || 0,
    // Maximum pool size for Redis connection pool
    pool: +process.env.BEAM_DEV_REDIS_POOL || 20,
    // Value to prepend to beamengine database keys
    prepend: process.env.BEAM_DEV_REDIS_PREPEND || 'beamengine',
    // Time (ms) before expiry of packet data
    packetTTL: +process.env.BEAM_DEV_REDIS_PACKET_TTL || 120000,
    // Time (ms) before expiry of frame data
    frameTTL: +process.env.BEAM_DEV_REDIS_FRAME_TTL || 10000,
    // Time (ms) before expiry of ephemeral data blob
    ephemeralTTL: +process.env.BEAM_DEV_REDIS_EPHEMERAL_TTL || 10000,
    // Time (ms) before error when closing Redis pool
    closeTimeout: +process.env.BEAM_DEV_REDIS_CLOSE_TIMEOUT || 2000
  },
