...
async function formatRoute (ctx) {
  ...
  try {
    if (ctx.params.fmtSpec.endsWith('.json')) {
      ctx.params.fmtSpec = ctx.params.fmtSpec.slice(0, -5);
    }
...
async function frameDataRoute (ctx) {
  ...
  try {
    ...
    if (Buffer.isBuffer(media)) {
      ...
      ctx.set('Beam-Buf-Sizes', JSON.stringify([ media.length ]));
      ctx.body = media;
    } else {
      if (!Array.isArray(media) || !media.every(Buffer.isBuffer)) {
        throw new Error('Result is not an array of buffers.');
      }
      let bufSizes = media.map(x => x.length);
      ctx.set('Beam-PTS', ctx.params.pts);
      ctx.set('Beam-Data-Index', ctx.params.idx);
      ctx.set('Beam-Buf-Sizes', JSON.stringify(bufSizes));
      ctx.body = Buffer.concat(media, bufSizes.reduce((l, r) => l + r, 0));
    }
  } catch (err) {
...
async function equivalentQuery (ctx) {
  console.log(ctx.params);
}

async function createEquivalent (ctx) { // eslint-disable-line no-unused-vars

}

async function deleteEquivalent (ctx) { // eslint-disable-line no-unused-vars

}

async function renditionQuery (ctx) { // eslint-disable-line no-unused-vars

}

async function createRendition (ctx) { // eslint-disable-line no-unused-vars

}

async function deleteRendition (ctx) { // eslint-disable-line no-unused-vars

}

async function transformationQuery (ctx) { // eslint-disable-line no-unused-vars

}

async function createTransformation (ctx) { // eslint-disable-line no-unused-vars

}

async function deleteTransformation (ctx) { // eslint-disable-line no-unused-vars
