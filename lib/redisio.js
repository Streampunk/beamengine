...
async function storePacket(fmt, packet) {
  ...
  let result3 = await Promise.all([
    redis[0].pexpire(idxk, config.redis.packetTTL),
    redis[1].pexpire(key, config.redis.packetTTL) ]);
  if (delResult == 1 && result2[0] === 0) { result2[0] = 1; }
  redis.forEach(x => redisPool.recycle(x));
  return zip(result1, result2, result3).map(x =>
...
async function storePacketData(fmt, stream_index, pts, data) {
  ...
  let result = await redis[0].setBuffer(datak, data, 'PX', config.redis.packetTTL);
  redis.forEach(x => redisPool.recycle(x));
