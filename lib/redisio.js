...
async function createTransformation (source, target, recipe = '', bounds = '') { // eslint-disable-line no-unused-vars
  ...
  let transRecipe = `${config.redis.prepend}:trans_receipe:${transName}`;
  ...
  for ( let s of transRev ) {
    sourceWork = sourceWork.then(x => {
      return sourceRedis.sadd(s, target).then(y => y > 0 ? x + 1 : x);
    });
  }
  ...
  let result = await Promise.add([sourceWork, targetWork]);
