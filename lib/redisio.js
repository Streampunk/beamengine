...
async function deleteEquivalent (source, target) {
  ...
  let result = await Promise.all([
    ...
    targetRedis.sren(targetSet, source) ]);
  recycle;
  return result;
}
...
async function queryEquivalent (source, depth = 1) {
  ...
  // } else {
  //   let equivSets = equivSetNames.map(x => `${config.redis.prepend}:equivalent:${x}`);
  //   let redis = await redisPool.use();
  //   console.log(equivSetNames);
  //   let members = await redis.sunion.apply(redis, equivSets);
  //   redisPool.recycle(redis);
  //   return members;
  // }
}
...
async function queryRendition (target, depth = 1) { // eslint-disable-line no-unused-vars

}

async function reverseRendition (source, depth = 1) { // eslint-disable-line no-unused-vars
