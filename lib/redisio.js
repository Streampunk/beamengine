...
async function storePacketData(fmt, stream_index, pts, data) {
  ...
  redis.forEach(x => redisPool.recycle(x));
  return `${result}-${dataLength > 0 ? 'ovw' : 'crt'}`;
}
...
async function storeFrameData(fmt, stream_index, pts, data, idx = -1) {
  ...
  if (idx === -1) {
    for ( let x = 1 ; x < redis.length ; x++ ) {
      existsWork.push(redis[x].strlen(`${datak_base}:data_${x - 1}`));
    }
  } else {
    existsWork.push(redis[1].exists(`${datak_base}:data_${idx}`));
  }
  let [ frameExists, ...dataLengths ] = await Promise.all(existsWork);
  if (frameExists === 0) {
    redis.forEach(x => redisPool.recycle(x));
    throw new Error(`Packet metadata for given packet payload data '${key}' does not exist.`);
  }
  let setWork = [];
  if (idx === -1) {
    for ( let x = 0 ; x < data.length ; x++) {
      setWork.push(redis[x].setBuffer(`${datak_base}:data_${x - 1}`), data[x]);
    }
  } else {
    setWork.push(redis[0].setBuffer(`${datak_base}:data_${idx}`), data[0]);
  }
...
async function retrieveFrameData(fmtOrKey, stream_index = 0, pts = 0, idx = '') {
  ...
  if (idx.length === 0) { // Get all data and concatenate
    ...
    bufSizes = result.map(x => x.length);
    return {
      data: Buffer.concat(result, bufSizes.reduce((l,r) => l + r, 0)),
      bufSizes
    };
  }
