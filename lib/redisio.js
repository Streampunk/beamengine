    ...
    if (count > 0) {
      ...
      for ( let x = curLen ; x < this.pool.length ; x++ ) {
        this.pool[x] = new Redis({
          ...
          db: this.testing ? config.redis.db : config.testing.db
        });
      }
    }
    return true;
...
['pool', 'nextFree', 'queue', 'grow', 'closeCheck', 'closeResolve'].forEach(p => {
...
async function listContent(start = 0, end = -1) {
  ...
  let list = await redis.lrange(`${config.redis.prepend}:index`, start, end);
...
async function retrieveMedia(fmt, stream_index, pts_start,
  pts_end = Number.MAX_SAFE_INETEGR, offset = 0, limit = 10) {
  let redisZero = await redisPool.use();
  let basek =  `${config.redis.prepend}:${typeof fmt === 'string' ? fmt : fmt.url}` +
    `:stream_${stream_index}:`;
  let idxk = `${basek}:index`;
  let mediaElements =
    redisZero.zrangebyscore(idxk, pts_start, pts_end, 'limit', offset, limit);
  redisPool.recycle(redisZero);
