...
async function createRendition (source, target, streamMap = {}) { // eslint-disable-line no-unused-vars
  if (source.url && typeof source.url === 'string') {
    source = source.url;
  }
  if (target.url && typeof target.url === 'string') {
  ...
  let recorded = await sourceRedis.sismember(sourceSet, target);
  if (recorded === 1) {
    recycle();
    throw new Error(`Target '${target}' is already recorded as a rendition of source '${source}'.`);
  }
  let result = await Promise.all([
    sourceRedis.sadd(sourceSet, target),
...
async function deleteRendition (source, target) { // eslint-disable-line no-unused-vars
  if (source.url && typeof source.url === 'string') {
    source = source.url;
  }
  if (target.url && typeof target.url === 'string') {
  ...
  let sourceSet = `${config.redis.prepend}:rendition_deps:${source}`;
  let targetKey = `${config.redis.prepend}:rendition:${target}`;
  // Don't delete stream map ... just overwrite if it comes back
  let result = await Promise.all([
    sourceRedis.srem(sourceSet, target),
...
async function getSubRenditions(target, depth, parents) {
  parents.push(target);
  let redis = await redisPool.use();
  let targetKey = `${config.redis.prepend}:rendition_rev:${target}`;
  let source = await redis.get(targetKey);
  redisPool.recycle(redis);
  if (depth > 1) {
    let subNames = await getMemberNames(source, depth - 1, parents);
...
async function queryRendition (target, reverse = false, depth = 1) { // eslint-disable-line no-unused-vars
  ...
  let subs = getSubRenditions(target, depth, []);
  let redis = await redisPool.use();

  let streamMap = redis.hgetall(`${config.redis.prepend}:rendition_map:${target}`);
...
async function createTransformation (source, target, recipe = '', bounds = '') { // eslint-disable-line no-unused-vars

}

async function deleteTransformation (source, target) { // eslint-disable-line no-unused-vars

}

async function queryTransformation (source) { // eslint-disable-line no-unused-vars

}

async function reverseTransformation (source) { // eslint-disable-line no-unused-vars
